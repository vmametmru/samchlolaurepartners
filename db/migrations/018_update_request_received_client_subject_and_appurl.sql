...
-- pointing at the install-time default localhost URL instead of the live
-- production site. Only touches rows that were never customised away from
-- that default.
UPDATE settings
SET value = 'https://www.grand-baie-maurice.com',
    updated_at = NOW()
WHERE `key` = 'APP_URL'
  AND value = 'http://localhost:8080';

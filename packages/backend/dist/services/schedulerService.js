...
async function processScheduledEmails() {
  ...
  `);
    for (const row of rows) {
        ...
        const [templateRows] = await connection_1.default.execute('SELECT * FROM email_templates WHERE partner_id = ? AND type = ? LIMIT 1', [row.partner_id, row.template_type]);
        ...
        try {
            await (0, emailService_1.sendTemplatedEmail)(partner, template, row.client_email, variables);
            // Mark as sent
            await connection_1.default.execute('INSERT IGNORE INTO sent_schedule_emails (schedule_id, reservation_id) VALUES (?, ?)', [row.schedule_id, row.reservation_id]);

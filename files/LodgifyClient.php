        ...
        foreach ($roomTypeCalendars as $roomTypeCalendar) {
            ...
            foreach ($periods as $period) {
                ...
                if ($periodStart >= $periodEnd || $periodEnd <= $rangeStart || $periodStart >= $rangeEnd) {
                    ...
                    // Lodgify's "end" is the exclusive checkout boundary: the
                    // checkout day itself is not an unavailable night (a new
                    // guest can arrive that same day), so the day-by-day loop
                    // below must stop before it, not include/extend past it.

...
final class PartnersController extends Controller
{
    public static function index(): never
    {
        ...
        $rows = Database::connection()->query('SELECT id, subdomain, name, logo_url, primary_color, email, phone, facebook_url, tiktok_url, instagram_url, checkin_info_url_fr, checkin_info_url_en, markup_percent, cleaning_fee_per_person_per_night, tourist_tax_per_person_per_night, active, created_at, updated_at FROM partners ORDER BY name')->fetchAll(PDO::FETCH_ASSOC);
        self::json(['data' => $rows]);
    }
    ...
    public static function create(): never
    {
        ...
        try {
            $stmt = Database::connection()->prepare(
                'INSERT INTO partners (subdomain, name, logo_url, primary_color, email, phone, facebook_url, tiktok_url, instagram_url, checkin_info_url_fr, checkin_info_url_en, markup_percent, cleaning_fee_per_person_per_night, tourist_tax_per_person_per_night, smtp_host, smtp_port, smtp_user, smtp_pass)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([
                ...
                (string) $input['email'],
                self::nullableString($input['phone'] ?? null),
                self::nullableString($input['facebook_url'] ?? null),
                self::nullableString($input['tiktok_url'] ?? null),
                self::nullableString($input['instagram_url'] ?? null),
                self::nullableString($input['checkin_info_url_fr'] ?? null),
                self::nullableString($input['checkin_info_url_en'] ?? null),
    ...
    public static function update(int $id): never
    {
        ...
        try {
            $stmt = Database::connection()->prepare(
                'UPDATE partners SET name = ?, logo_url = ?, primary_color = ?, email = ?, phone = ?, facebook_url = ?, tiktok_url = ?, instagram_url = ?, checkin_info_url_fr = ?, checkin_info_url_en = ?, markup_percent = ?, cleaning_fee_per_person_per_night = ?, tourist_tax_per_person_per_night = ?, smtp_host = ?, smtp_port = ?, smtp_user = ?, smtp_pass = ?, active = ?, updated_at = NOW() WHERE id = ?'
            );
            $stmt->execute([

 ...
 *
 * Each block below is rendered as its own titled card (.package-admin-section)
 * so the long form reads as a sequence of clearly separated steps instead of
 * one continuous wall of fields (see assets/css/styles.css for the styling).
 */
...
$meals = $package['meals'] ?? [];
// "Transport", "Activités" et "Restauration" share exactly the same row
...
<section class="container section-lg">
  ...
  <form method="post" action="<?= $e($basePath) ?>" enctype="multipart/form-data" class="mt-16 stack-lg" id="package-form">
    ...
    <div class="card card-body package-admin-section">
      ...
      <div class="package-admin-photo-field">
        <label><span>Photo principale</span><input class="input" type="file" name="photo" accept="image/*"></label>
        <?php if (!empty($package['photo_url'])): ?>
          <img src="<?= $e($package['photo_url']) ?>" alt="" class="gallery-thumb package-admin-photo-preview">
        <?php endif; ?>
      </div>
      ...
      <div class="stack-md" data-package-rows="flights">
        <?php foreach ($flights as $index => $flight): ?>
          <div class="card card-body package-admin-row" data-package-row>
            ...
            <input type="hidden" name="flights[<?= (int) $index ?>][photo_url]" value="<?= $e($flight['photo_url'] ?? '') ?>">
            <div class="package-admin-photo-field">
              <label><span>Photo</span><input class="input" type="file" name="flights[<?= (int) $index ?>][photo]" accept="image/*"></label>
              <?php if (!empty($flight['photo_url'])): ?>
                <img src="<?= $e($flight['photo_url']) ?>" alt="" class="gallery-thumb package-admin-photo-preview">
              <?php endif; ?>
            </div>
            <div class="package-admin-row-footer">
    ...
    <?php foreach ($extraBlocks as $blockKey => $block):
      ...
      <div class="card card-body package-admin-section">
        <h2 class="section-title"><?= $e($block['icon'] ?? '') ?> <?= $e($block['title']) ?></h2>
        <p class="muted"><?= $e($block['hint']) ?></p>
        <div class="stack-md" data-package-rows="<?= $e($blockKey) ?>">
            ...
            <div class="card card-body package-admin-row" data-package-row>
              ...
              <div class="package-admin-photo-field">
                ...
                <?php if (!empty($row['photo_url'])): ?>
                  <img src="<?= $e($row['photo_url']) ?>" alt="" class="gallery-thumb package-admin-photo-preview">
                <?php endif; ?>
              </div>
              <div class="package-admin-row-footer">
...
<template data-package-template="flights">
  <div class="card card-body package-admin-row" data-package-row>
    ...
    <input type="hidden" name="flights[__INDEX__][photo_url]" value="">
    <div class="package-admin-photo-field">
      <label><span>Photo</span><input class="input" type="file" name="flights[__INDEX__][photo]" accept="image/*"></label>
    </div>
    <div class="package-admin-row-footer">
